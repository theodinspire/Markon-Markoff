...
class Viterbi {
    ...
    func getNaiveTagSequence(for words: [Word]) -> [Tag] {
        ...
        for word in sentence {
            ...
            if probabilities.isEmpty {
                //  TODO: Deal with unknown words/smoothing
                print("it empty")
            }
    ...
    func getNaiveTagSequence(for pairs: [WordTagPair]) -> [Tag] {
        let words = pairs.map() { $0.word }
        return getNaiveTagSequence(for: words)
    }
    
    func getForeBackTagSequence(for words: [Word]) -> [Tag] {
        var certainTags = [Tag?]()
        
        for word in words {
            let tag = emissor.singleTaggedWords[word]
            certainTags.append(tag)
            print("\(word) \(tag ?? "nil")")
        }
        //  Returns nil if the word is absent, which is what is wanted
        
        return []
    }
    
    func getForeBackTagSequence(for pairs: [WordTagPair]) -> [Tag] {
        let words = pairs.map() { $0.word }
        return getForeBackTagSequence(for: words)
    }
